...
def Voucher_Debit_Note(request):
     if 't_id' in request.session:
        if request.session.has_key('t_id'):
            t_id = request.session['t_id']
        else:
        ...
        vou1=Voucher.objects.filter(company_id=t_id)
        data=stock_itemcreation.objects.all()
        debit_note=Vouchers_Debit_Note.objects.all()
       
       
        
        return render(request,'Vouchers_Debit_Note.html',
...
def add_debit_note(request):
    if 't_id' in request.session:
        ...
        if request.method=='POST':
            ...
            Debit=Vouchers_Debit_Note(vou_type1=vtype,
                    pur_no=pur_no,
